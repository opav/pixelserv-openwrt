...
#define SECOND_PORT "443"	// used if MULTIPORT define
...
#ifdef MULTIPORT
	sockfd1 = sockfd;
/* also listen on the SSL port 443 */
	rv = getaddrinfo(use_ip ? ip_addr : NULL, SECOND_PORT, &hints, &servinfo);
	if (rv != OK) {
		syslog( LOG_ERR, "getaddrinfo 2: %s", gai_strerror(rv) );
		exit(EXIT_FAILURE);
	}
	
	if ( (( sockfd2 = socket(servinfo->ai_family, servinfo->ai_socktype, servinfo->ai_protocol) ) < 1)
		|| ( setsockopt(sockfd2, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) != OK )
		|| ( setsockopt(sockfd2, SOL_TCP, TCP_NODELAY, &yes, sizeof(int)) != OK )
#ifdef IF_MODE
		|| ( setsockopt(sockfd2, SOL_SOCKET, SO_BINDTODEVICE, ifname, IFNAMSIZ) != OK )
#endif
...
#ifdef MULTIPORT
	syslog(LOG_NOTICE, "Also Listening on %s:%s", ip_addr, SECOND_PORT);
#endif
...
/* MACRO needs "_GNU_SOURCE", without this the select gets interrupted with errno EINTR */
		select_rv = TEMP_FAILURE_RETRY( select(sockfd2 + 1, &readfds, NULL, NULL, NULL) );
		if (select_rv < 0) {
			perror("select(fd1,2) error:");
			exit(EXIT_FAILURE);
		}
